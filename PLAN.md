# ferromesh — MeshCore ingest, store, watch, alert

Status (2026-09-19):
- **Phase 0 complete:** decoder crate plus golden test.
- **Phase 1 complete:** a week of Tanyard's traffic arrived without loss.
...
- **Phase 5b complete:** sending works; the companion radio runs on truffles since 2026-09-19.
- **Next:** 5d (radio configuration, including the channel slots left from 5c), 5e (console, which gives Tanyard telemetry over the air), then 6 (Home Assistant).
- **2026-09-19:** `[mqtt]` became optional, so a companion radio alone is a complete install; the documentation moved to `docs/`.
- **2026-09-21:** the delivery check knows what a companion radio can report: its firmware never hands over a packet over 173 bytes (`len + 3 <= MAX_FRAME_SIZE`, 176), which was the whole of `scw`'s 2-3% shortfall — measured against Tanyard, which stored packets up to 241 bytes off the same air. Observers now record how we hear from them (`mqtt` or `companion`), and the delivery warning allows 95% for a companion against 99% for a publisher. Not the database: SQLite stored 15,258 of 15,260 records that day, and an hour at debug level skipped zero bytes.
- **2026-09-20:** `ferromesh advert` and the TUI's `a`, and the TUI's DMs view (`2`): a conversation per correspondent, with `c` to reply. Direct messages aren't stream events, so the view polls every 10 seconds.
...
| Sources | **Either or both:** an MQTT broker fed by observer repeaters, and a companion radio on USB. At least one is required. A radio alone is a complete install — its own vantage point, DMs and sending — which is the lowest-effort way for someone else to try ferromesh (added 2026-09-19). |
...
- **A companion radio can't report every packet.** Its firmware logs a reception only when the frame fits the serial protocol (`len + 3 <= MAX_FRAME_SIZE`, 176), so nothing over 173 bytes ever reaches us, though the radio counts it. Measured 2026-09-21: 2.2% of this mesh's traffic. Any "did we get everything" check has to allow for it.
...
| 5b ✅ | **Send.** `POST /api/v1/send` (token), an outbox (queued → sent → delivered or failed, with round-trip time), matching each send against what observers heard, a TUI compose line and `ferromesh send`. Then move the radio to truffles. Built 2026-09-19: channel sends take a radio slot on first use and carry a precomputed packet hash, so the outbox counts every observer's reception; direct sends add the recipient as a contact from its advert and track the ACK; the radio's clock is set when behind; sends and ACKs go through the raw log. Verified live on the Mac 2026-09-19: a DM to KK4SW was acknowledged in 0.6 s, and `#test` "hello from ferromesh" was heard 4 times (Tanyard at 0 and 2 hops, `scw` at 1 and 2), matching the precomputed hash. Moved to truffles 2026-09-19 18:17 UTC (e795713): the radio is passed into the container by a local `compose.override.yaml`, and its contacts, pins and contact policy carried over. | Send to `#test` from the TUI and see it come back through Tanyard, with a delivery report |
| 5c 🟡 | **Radio management.** Keep the radio's channel slots in step with the server's list, contact policy, clock sync, advert schedule, and Tanyard status and telemetry via repeater login. Done 2026-09-19, ahead of the move to truffles: the contact policy (auto-add chat radios only, overwrite the oldest non-favourite when full; the radio holds 350, and the mesh had 1,132 nodes in six days, 967 of them repeaters), favourites for everyone you exchange DMs with and `ferromesh contacts pin`, rescue of DMs the radio couldn't read, and `ferromesh contacts`. Clock sync came with 5b. Also 2026-09-19: observer health from the status reports observers already send (`ferromesh health`, the TUI's Health view, `GET /api/v1/observers`), with a delivery check and warnings; no over-the-air requests needed for Tanyard. Test whether `CMD_SEND_RAW_PACKET` can send to channels without a slot. | Tanyard's battery and noise floor are recorded every N minutes |
| 5d 🟡 | **Radio configuration.** Advertising on demand is done (2026-09-20): `ferromesh advert [--flood]`, `a` in the TUI, `POST /api/v1/advert`, with a flood refused within a minute of the last. Still to do — `ferromesh radio` (and a TUI panel later): show the companion's settings and change them — advertised name, latitude/longitude, frequency/bandwidth/SF/CR, transmit power, a scheduled advert, reboot — plus the channel-slot sync left over from 5c, so the radio's 40 slots track the server's channel list. Opcodes confirmed 2026-09-24 from meshcore_py's `CommandType`: `SET_ADVERT_NAME` 8, `SET_RADIO_PARAMS` 11, `SET_RADIO_TX_POWER` 12, `SET_ADVERT_LATLON` 14, `REBOOT` 19. Also here, from the same list: `REMOVE_CONTACT` 15, which the contacts view has no way to do (it can only pin and unpin), and `GET_CONTACTS` taking an optional `since`, so the TUI stops refetching all 116 contacts every minute. Changing radio parameters can take the node off the mesh silently, so those need the token and a confirmation, and should warn when they disagree with what the rest of the mesh uses. | `ferromesh radio` shows the radio's settings; changing the name and re-advertising is visible from another node, and a channel added on the server appears in the radio's slots |
| 5e 🟡 | **Login: rooms, remote admin, console.** One mechanism, three uses. `SEND_LOGIN` 26 is the destination's 32-byte key plus the password; the reply is a `Sent` frame with a suggested timeout, then `LOGIN_SUCCESS` 0x85 (permissions byte, 6-byte key prefix, admin bit) or `LOGIN_FAILED` 0x86; `LOGOUT` is 29. A radio forgets its logins when it reboots, so ferromeshd logs in again on connect, as it re-applies the contact policy. Passwords live in the server's config and never reach the raw log. **Rooms:** a room server stores posts and re-broadcasts them to members, so it's a channel socially and direct messages on the wire — no room-specific commands exist. Most of it is already here: `txt_type` 2 (`SIGNED_PLAIN`) carries a 4-byte prefix of the author's key, and those posts already land in `direct_messages` with `signer_prefix` set. What's missing is the login, naming the author from that prefix (only when exactly one known node matches, as for DM senders), and showing a room beside the channels rather than as a DM thread. Posting needs nothing new: a post is a plain text DM to the room server, which re-broadcasts it. Joining is deliberate — the auto-add policy takes only chat radios — so pinning the room server is the join. **Remote admin:** the same login against a repeater, then `SEND_STATUS_REQ` 27 (`STATUS_RESPONSE` 0x87) for telemetry without a broker, and CLI over the air. **Console:** `RUN_CLI_COMMAND` 66 with its `CLI_REPLY` 29 for the attached radio. Command and reply are recorded as an audit trail; passwords are not. | A room's posts appear beside the channels with their authors named, a post from the TUI reaches the room, and `get stats` to Tanyard returns its figures without MQTT |
| 5f ✅ | **Messaging and contacts in the TUI.** Done 2026-09-21. Contacts: view 8 lists the radio's contacts in its own eviction order, `p` keeps or releases one, `m` writes to one, and `p` in the Nodes view adds a node. Starting a conversation: `n` in the DM view takes a name or key prefix and opens an empty conversation to write in, and `m` does it from the nodes, contacts or messages list. Replying: `r` answers the selected channel message on its own channel, prefilled with `@[Sender] `, and mentions render as `@Sender` everywhere a body is shown, brackets being wire-only (`meshcore_proto::mention`). The three things the DM view showed were missing (2026-09-20): (1) **contacts management** — a panel over `GET`/`POST /api/v1/contacts` to see what the radio holds, pin and unpin, add a node from the Nodes view, and show which contact the radio would replace next, since it holds 350 and the mesh has thousands; (2) **starting a conversation** — `c` in the DM view only replies to someone already there, so picking any node that has advertised and writing to it needs a picker, probably over the Nodes view; (3) **replying to a channel message** — with a message selected, compose a reply to that channel addressed to its sender, or send it as a direct message instead when an advert has given us the sender's key. Other MeshCore clients appear to write a mention as `@[Sender]:`; confirm the exact form against the MeshCore app and [MeshMonitor](https://meshmonitor.org/features/meshcore.html) before implementing, because a reply nobody else's client highlights is worth nothing. | Pin a contact, start a DM with someone new, and reply to a channel message from the TUI alone |
| 6 | **Alerts that leave the terminal.** `meshcore_proto::mention` can already tell when a message names your radio (built 2026-09-21, unused): treat that like a watch match, then the **Home Assistant bridge** (marbles.local): connect Home Assistant to the Mosquitto on truffles, then publish sensors, watch events and a send command through MQTT discovery | A watch match reaches your phone, and an automation posts to a channel |
| 7 | Web UI (stack TBD) | Live feed + chat + search + nodes |
| 8 | Analytics and more sources: coverage and path maps — with `SEND_TRACE_PATH` 36 and `PATH_DISCOVERY` 52 the radio can trace a route rather than only infer one from what it overhears — node health, NashMesh as an observer, optional forwarding to community maps, retention/Parquet archive | as scoped then |

### Housekeeping, not on the critical path

- **Releases.** The repository is public with no tag, no CHANGELOG and no binary: the only way in is `cargo install --path`. Anyone else wanting to run it has to build the workspace.
- **CI can't check the decoder.** The golden fixture comes from a real capture that stays off GitHub, so those tests skip in CI. A small synthetic capture, generated rather than recorded, would let CI verify the decoder on every push.
- **Nothing is backed up.** Truffles holds a 36 MB database and a growing raw log with no copy off the machine. The raw log makes the database rebuildable, which is worth nothing if the disk takes both.
...
(Answered 2026-09-21: the mention format is `@[Name]` on the wire, shown as `@Name`. MeshCore documents none — its app is closed and #618 closed without one — but MeshMonitor, meshcadet and MeshCoreOne all implement that form independently, which is what makes ours recognisable to them.)
...
- meshcore_py — the companion command list (`CommandType`) and push codes (`PacketType`), read 2026-09-24: https://github.com/meshcore-dev/meshcore_py/blob/main/src/meshcore/packets.py
