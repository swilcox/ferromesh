...
| 5d 🟡 | **Radio configuration.** Advertising on demand is done (2026-09-20): `ferromesh advert [--flood]`, `a` in the TUI, `POST /api/v1/advert`, with a flood refused within a minute of the last. Still to do — `ferromesh radio` (and a TUI panel later): show the companion's settings and change them — advertised name, latitude/longitude, frequency/bandwidth/SF/CR, transmit power, a scheduled advert, reboot — plus the channel-slot sync left over from 5c, so the radio's 40 slots track the server's channel list. Needs new codec commands (`SET_ADVERT_NAME`, `SET_RADIO_PARAMS`, `SET_TX_POWER`, `SET_ADVERT_LATLON`, `SEND_SELF_ADVERT`, `REBOOT`); confirm each opcode against the firmware source and meshcore-cli rather than assume. Changing radio parameters can take the node off the mesh silently, so those need the token and a confirmation, and should warn when they disagree with what the rest of the mesh uses. | `ferromesh radio` shows the radio's settings; changing the name and re-advertising is visible from another node, and a channel added on the server appears in the radio's slots |
| 5e | **Console.** `ferromesh console` opens the attached radio's own CLI; `ferromesh console Tanyard` logs in to a repeater over the air and runs commands there, which is how a radio-only install gets a repeater's telemetry with no MQTT at all. The mechanism is mostly present: `txt_type::CLI_DATA` already exists, so it is a login command plus direct messages of that type, with replies collected and stored. Repeater passwords come from the client config or the environment and never reach the raw log; command and reply are recorded as an audit trail. | A `get stats` to Tanyard from the terminal returns its figures, and its battery shows up in `ferromesh health` without a broker |
